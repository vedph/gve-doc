...
So, there are 2 staged versions: an intermediate one (v3=`alpha`) and a final one (v5=`beta`). If we now run the same rendering pipeline illustrated [above](#example-1) on this snapshot, assuming that the final version is our preferred one we get this TEI code:
