...
- `units` (`ContentUnit[]`):
  ...
  - `note` (`string`)
  - `features` (`NamedValue[]`, 📚 `asserted-id-features`, hierarchical):
    - `name`\* (`string`)
    - `value`\* (`string`)
...
As for data browsing, essentially the entities at play are:

- carriers (being material objects)
- snapshots (having a material part)
- versions (abstractions generated by snapshots; these are the texts)
