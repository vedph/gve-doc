...
A popular TEI method for representing multiple versions of a text while avoiding overlap is represented by the so-called "parallel segmentation method" used for critical texts. This has been proposed for the autographs variants as a highly compatible encoding to be consumed in legacy tools. In this method, the texts compared are split into matching segments in synch with one another. This makes it easy to compare different readings side by side, and also to extract the full text of any witness (or version, in our scenario). Of course, its drawback is that this encoding may quickly become highly nested and redundant, and there might be issues when trying to adopt different levels of granularity in segmenting text.

In this method, we wrap variants in an `app` element, including each one in `rdg` (or in `lem` when this is a preferred variant).So, to render an XML based on a binary alternative like in the example of `app` including `lem` and `rdg`, we need to further constrain the above structure to allow at most two children per node.
